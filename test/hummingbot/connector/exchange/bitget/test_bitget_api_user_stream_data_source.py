...
class BitgetAPIUserStreamDataSourceTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        auth = BitgetAuth(
            api_key="TEST_API_KEY", secret_key="TEST_SECRET", passphrase="PASSPHRASE", time_provider=TimeSynchronizer()
        )
    ...
    def _authentication_response(self, authenticated: bool) -> str:
        message = {"event": "login" if authenticated else "err", "code": "0" if authenticated else "4000", "msg": ""}
    ...
    def _subscription_response(self, subscribed: bool, subscription: str) -> str:
        message = {
            "event": "subscribe",
            "arg": [{"instType": "SPOT", "channel": subscription, "instId": self.ex_trading_pair}],
        }
    ...
    def test_listening_process_authenticates_and_subscribes_to_events(self, ws_connect_mock):
        ...
        self.mocking_assistant.add_websocket_aiohttp_message(
            ...
            self._subscription_response(True, CONSTANTS.WS_SUBSCRIPTION_TRADE_CHANNEL_NAME),
        )
        self.mocking_assistant.add_websocket_aiohttp_message(
            ws_connect_mock.return_value,
            self._subscription_response(True, CONSTANTS.WS_SUBSCRIPTION_ORDERS_CHANNEL_NAME),
        )
        self.mocking_assistant.add_websocket_aiohttp_message(
            ws_connect_mock.return_value,
            self._subscription_response(True, CONSTANTS.WS_SUBSCRIPTION_ACCOUNT_CHANNEL_NAME),
        )
        ...
        self.assertTrue(self._is_logged("INFO", "Subscribed to private account, trade and orders channels..."))
        ...
        expected_payload = {
            ...
            "args": [
                ...
                {
                    "instType": "SPOT",
                    "channel": CONSTANTS.WS_SUBSCRIPTION_ACCOUNT_CHANNEL_NAME,
                    "coin": "default",
                },
                {
                    "instType": "SPOT",
                    "channel": CONSTANTS.WS_SUBSCRIPTION_TRADE_CHANNEL_NAME,
                    "instId": "default",
                },
            ],
        }
