...
class XtUserStreamDataSourceUnitTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.auth = XtAuth(api_key="TEST_API_KEY", secret_key="TEST_SECRET", time_provider=self.mock_time_provider)
        ...
        self.data_source = XtAPIUserStreamDataSource(
            auth=self.auth,
    ...
    def _user_update_event(self):
        ...
        resp = {
            "topic": "balance",
            "event": "balance",
            "data": {"a": "123", "t": 1656043204763, "c": "btc", "b": "123", "f": "11", "z": "SPOT", "s": "btc_usdt"},
        }
    ...
    def _get_listen_key(self):
        resp = {
            ...
            "ma": [],
            "result": {
                "accessToken": "TEST_LISTEN_KEY",
                "refreshToken": "TEST_LISTEN_KEY",
            },
        }
        return resp
    ...
    def test_get_listen_key_log_exception(self, mock_api):
        url = web_utils.private_rest_url(path_url=CONSTANTS.GET_ACCOUNT_LISTENKEY, domain=self.domain)
    ...
    def test_get_listen_key_successful(self, mock_api):
        ...
        mock_response = self._get_listen_key()
    ...
    def test_ping_listen_key_successful(self, mock_api):
        url = web_utils.private_rest_url(path_url=CONSTANTS.GET_ACCOUNT_LISTENKEY, domain=self.domain)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        mock_api.post(regex_url, body=json.dumps(self._get_listen_key()))
    ...
    @patch(
        "hummingbot.connector.exchange.xt.xt_api_user_stream_data_source.XtAPIUserStreamDataSource" "._ping_listen_key",
