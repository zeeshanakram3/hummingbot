...
class XtExchange(ExchangePyBase):
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            async with timeout(timeout_seconds):
                ...
                successful_cancellations = [
                    CancellationResult(client_order_id, True) for client_order_id in successful_cancellations
                ]
                failed_cancellations = [
                    CancellationResult(client_order_id, False) for client_order_id in failed_cancellations
                ]
        except Exception:
            self.logger().network(
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        for rule in filter(xt_utils.is_exchange_information_valid, trading_pair_rules):
            try:
                ...
                min_order_size = (
                    Decimal(quantity_size_filter["min"])
                    if quantity_size_filter is not None and quantity_size_filter.get("min") is not None
                    else Decimal("0")
                )
                min_notional_size = (
                    Decimal(quote_qty_size_filter["min"])
                    if quote_qty_size_filter is not None and quote_qty_size_filter.get("min") is not None
                    else Decimal("0")
                )
    ...
    async def _all_trade_updates_for_order(self, order: InFlightOrder) -> List[TradeUpdate]:
        ...
        if order.client_order_id not in self._order_tracker.cached_orders:
            ...
            response = await self._api_get(
                ...
                params={"symbol": trading_pair, "orderId": int(exchange_order_id)},
                is_auth_required=True,
    ...
    async def _get_last_traded_price(self, trading_pair: str) -> float:
        ...
        return float(resp_json["result"]["c"])
